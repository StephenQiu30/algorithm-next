...
export const useSortingVisualizer = (initialSize: number = 10, initialAlgorithmId?: SortingAlgorithmId) => {
  ...
  const applyStepIndex = useCallback(
    (nextIndex: number) => {
      ...
      if (clamped === steps.length) {
        ...
        setIsSorted(true);
        setActiveIndices([]);
        setSortedIndices(Array.from({ length: steps[steps.length - 1]?.array.length ?? array.length }, (_, i) => i));
        setCurrentStep(clamped);
        setMetrics(metricsPrefixRef.current[clamped] ?? metricsPrefixRef.current[metricsPrefixRef.current.length - 1]);
        return;
      }
